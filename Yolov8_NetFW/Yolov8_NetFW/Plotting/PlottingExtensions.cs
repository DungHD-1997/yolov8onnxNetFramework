...
namespace Yolov8_NetFW.Plotting
{
    public static class PlottingExtensions
    {
        ...
        public static Image PlotImage(this ISegmentationResult result, ImageSelector<Rgba32> originImage, SegmentationPlottingOptions options)
        {
            ...
            var contoursLayer = new Image<Rgba32>(size.Width, size.Height);
            ...
            for (int i = 0; i < result.Boxes.Count; i++)
            {
                ...
                var mask = new Image<Rgba32>(box.Bounds.Width, box.Bounds.Height);
                ...
                {
                    var contours = CreateContours(mask, color, options.ContoursThickness * ratio);
